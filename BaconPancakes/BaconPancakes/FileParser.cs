...
namespace BaconPancakes
{
    ...
    {
        public List<char[]> FilenameToList(String filename)
        {
            Console.WriteLine("Reading " + filename);

            string[] text = System.IO.File.ReadAllLines(filename);
            var isNumeric = int.TryParse(text[0], out int numberOfEdge);
            ...
            List<char[]> list_hasil = new List<char[]>();
            for (int i = 1; i <= numberOfEdge && text[i] != null; i += 1)
            {
                Console.WriteLine(text[i]);
                char[] x = text[i].Replace(" ", "").ToCharArray();
                if (x.Length > 2)
                {
        ...
        public UndirectedGraph ListToUndirectedGraph(List<char[]> list_hasil)
        {
            UndirectedGraph undirected_graph = new UndirectedGraph();
            foreach (char[] x in list_hasil)
            {
